...
//! SPEC.md §4·§6 이 문서화한 바이트를 그대로 구현한다(브라우저 사이드카가 코어를
//! 링크하지 않고 문서화된 엔진 ABI 를 구현하는 것과 같은 선례). 여기 상수·경로 파생·
...
/// 이 사이드카 유닛이 데몬 hello 에 싣는 client id(관측 라벨 — 인증은 토큰).
pub const CLIENT_ID: &str = "soksak-sidecar-terminal-alacritty";
...
// 싱글턴이 계약당 하나이도록 엔진 이름을 넣지 않는다 — wezterm 유닛이 켜지면 같은
// 경로를 프로브해 물러난다(SPEC §2 singleton). 프로토콜-키드.
