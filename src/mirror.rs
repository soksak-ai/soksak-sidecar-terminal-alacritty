...
impl Mirror {
    pub fn new(cols: u16, rows: u16) -> Self {
        let mut m = Mirror { engine: Engine::new(cols, rows), frozen_primary: None, held: Vec::new() };
        // 계약이 미러의 **출생 상태**를 선언한다(SPEC.md §11.I). 이 엔진의 파워온 기본값은 그
        // 선언과 한 곳에서 어긋난다 — alternate scroll(1007)을 켠 채로 태어난다. 계약의 선언은
        // xterm 의 alternateScroll 리소스 기본값 "false" 이므로, 태어나자마자 그리로 맞춘다.
        // 엔진의 기본값이 기준이 아니다 — 기준이 엔진을 맞춘다.
        m.engine.feed(b"\x1b[?1007l");
        m
    }
