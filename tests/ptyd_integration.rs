...
use soksak_sidecar_terminal_alacritty::daemon::{ControlClient, TeeFrame, TeeStream};
use soksak_sidecar_terminal_alacritty::mirror::Mirror;
use soksak_sidecar_terminal_alacritty::proto;
use soksak_sidecar_terminal_alacritty::service::ServiceClient;
...
fn fresh_home(tag: &str) -> PathBuf {
    ...
    let home = PathBuf::from(std::env::var("HOME").unwrap())
        .join(".soksak-e2e")
        .join(format!("ptyd-int-{tag}-{}-{nanos}", std::process::id()));
...
fn spawn_sidecar(home: &Path) -> Sidecar {
    let bin = env!("CARGO_BIN_EXE_soksak-sidecar-terminal-alacritty");
    Sidecar(
        Command::new(bin)
            .env("SOKSAK_HOME", home)
            .stdout(Stdio::null())
...
fn binary_runtime_serves_and_singleton() {
    ...
    let bin_path = env!("CARGO_BIN_EXE_soksak-sidecar-terminal-alacritty");
